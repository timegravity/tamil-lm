...
MODELS = [("Gemini 3.5 Flash-Lite", "google/gemini-3.5-flash-lite", "Google (Vertex), zero data retention"),
          ("GPT-5.4 nano", "openai/gpt-5.4-nano", "OpenAI, no zero retention (ruling 2026-09-13), no data collection"),
          ("gpt-oss-20b (reasoning could not be disabled: each request carried 1,024 extra tokens; not directly comparable)", "openai/gpt-oss-20b", "any provider, cheapest first with fallbacks, no data collection (ruling 2026-09-13); reasoning at low effort"),
          ("gpt-oss-120b (reasoning could not be disabled: each request carried 1,024 extra tokens; not directly comparable)", "openai/gpt-oss-120b", "any provider, cheapest first with fallbacks, no data collection (ruling 2026-09-13); reasoning at low effort")]
...
OURS = ("tamil-lm-2b-instruct (round 4c, this repository)", "cmp_tamil-lm-2b-instruct-r4_chat_test.json")
...
def main():
    head = "| model | served by | " + " | ".join(c[2] for c in COLS) + " | requests | cost (USD) | answer caps |"
    L = [f"# Hosted models, locked test split, generation tasks, chat mode ({time.strftime('%Y-%m-%d %H:%M UTC', time.gmtime())})", "",
         ...
         "each request pinned to the lab's own provider with fallbacks off (gpt-oss: any provider by ruling, cheapest first, provider recorded per response; it cannot switch reasoning off and runs at low effort, with 1,024 extra output tokens allowed for its reasoning), and the serving provider recorded on every response. Our model's row is its table (b) run on the same items.", "",
         head, "|" + "---|" * (len(COLS) + 5)]
    o = scores(OURS[1])
    ...
    L.append(f"| {OURS[0]} | local, bf16 | " + " | ".join(f"{need(o, (t, m), 'score'):.3f}" if m != "chrf++" else f"{need(o, (t, m), 'score'):.1f}" for t, m, _ in COLS) + " | | | suite v3 caps |")
    ...
    for name, model, policy in MODELS:
        if model in ANY_PROVIDER and not os.path.exists(os.path.join(R, f"hosted_{slug(model)}_chat_test.json")):
            continue   # ruled 2026-09-13, not run yet: the row appears when its result file lands
        s = scores(f"hosted_{slug(model)}_chat_test.json")
        raw = [json.loads(l) for l in open(os.path.join(R, "hosted_raw", slug(model) + ".jsonl"), encoding="utf-8") if l.strip()]
        n_items = sum(need(s, (t, m), "n") for t, m, _ in COLS if m in ("chrf++", "acc", "f1") and not (t == "indicqa_ta" and m == "f1")) + need(s, ("indicqa_ta", "f1"), "n")
        ...
        caps = {r["benchmark"]: r["gen_cap"] for r in s.values() if r.get("gen_cap")}
        ...
        L.append(f"| {name} | {served_txt} | " + " | ".join(f"{need(s, (t, m), 'score'):.3f}" if m != "chrf++" else f"{need(s, (t, m), 'score'):.1f}" for t, m, _ in COLS) + f" | {sum(served.values())} | {cost:.2f} | {cap_txt} |")
        import preamble_share as PS
