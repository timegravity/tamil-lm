...
- Tokenizer: 22,222 Tamil SentencePiece pieces added to the Qwen vocabulary, final vocabulary 270,299 (a loaded tokenizer reports 22,255 added tokens: those pieces plus the base model's 33 special tokens). Tamil text costs 1.92 tokens per word instead of 6.49. See Tokenizer extension under Training.
...
A SentencePiece unigram model was trained to a 24,000-piece target on Tamil web text. Pieces with no Tamil script were dropped, and of the rest a piece was added only where the Qwen tokenizer needed two or more tokens for that string, while anything Qwen already encoded as a single token was dropped. 22,222 pieces survived both filters: that is the post-filter count of added tokens, not the SentencePiece target, and it takes the vocabulary from 248,077 to 270,299. The base-tokenizer ids a kept piece split into became that token's embedding initialisation, which the embedding warmup below then trained.

Fertility (tokens per word) is measured on two texts. On 10,000 Tamil Wikipedia sentences, Tamil text costs 1.92 tokens per word against 6.49 with the original tokenizer, so a 4,096-token context holds about 2,100 Tamil words instead of about 630. On the FLORES benchmark text the same tokenizers cost 1.81 against 6.59, which is the pair the comparison tables report. Every number here is regenerated by eval/tokenizer_extension_report.py into eval/results/tokenizer_extension.json, which also pins the Wikipedia dump revision and a checksum of the sentence set.
